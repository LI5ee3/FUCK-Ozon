...
from app import db
from app.main import app, finance, orders, returns, rfbs_returns, stock, timeliness
...
class ModuleViewsTest(unittest.TestCase):
    def setUp(self):
        ...
        with db.transaction() as connection:
            ...
            connection.execute("INSERT INTO finance_records VALUES(1,'F-1','2026-08-01T00:00:00Z',?,?)",
                               (json.dumps({"amount": 3, "operation_type_name": "Оплата эквайринга"}),
    ...
    def test_each_view_reads_only_its_module_and_shop(self):
        views = [timeliness, finance, returns, rfbs_returns, stock]
        for view in views:
        ...
        finance_data = finance(1)
        self.assertEqual(finance_data["items"][0]["operation_type"], "收单服务费")
        self.assertEqual(finance_data["items"][0]["currency"], "RUB")
        self.assertEqual(finance_data["summary"]["shops"][0]["currency"], "RUB")
    ...
    def test_removed_modules_are_absent(self):
        ...
        self.assertNotIn("/api/premium", {route.path for route in app.routes})
        self.assertNotIn("/api/prices", {route.path for route in app.routes})
